...
Page {
    ...
    */
    ...
    function loadSettings() {
        sftp_plugin.load_settings()

        serverInputField.text = sftp_plugin.server
        portInputField.text = sftp_plugin.port
        pathInputField.text = sftp_plugin.path
        usernameInputField.text = sftp_plugin.username
        passwordInputField.text = sftp_plugin.password
    }
    ...
    Button {
        ...
        onClicked: {
                ...
                sftp_plugin.save_settings(serverdata, portdata, pathdata, userdata, passdata)

                if (serverdata === ""){
                    noServerWarningDialog.open();
                    sftp_plugin.delete_settings()
            }
        }
    }
